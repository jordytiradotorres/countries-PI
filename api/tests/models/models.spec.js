...
describe('Country model', () => {
  ...
  describe('Validators', () => {
    ...
    describe('name', () => {
      ...
      xit('should work when its a valid name', (done) => {
        Country.create({
          name: 'Argentina',
          capital: 'Buenos Aires',
          continent: 'Americas',
          ...
          .then(() => done())
          .catch(() =>
            done(new Error('This should not happen because the name is valid'))
          );
      });
    });
    describe('flag', () => {
  ...
  describe('creating activities', () => {
    ...
    it('should create an activity if all parameters are valid', (done) => {
      Activity.create({
        ...
        .then((results) =>
          results.length === 1 ? done() : new Error('wrong')
        ); //expect(results.length).to.be.equal(1)) //what?
